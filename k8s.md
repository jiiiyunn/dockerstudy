...
디플로이먼트의 전략(strategy)에 들어가지 않으며 ingress를 이용해서 구현된다
...
ingress는 일종의 패턴(라우팅 규칙)이며 이를 제어하는 것이 ingress controller
...
**클러스터 안의 가상 클러스터** - 파드, 레플리카셋, 디플로이먼트, 서비스와 같은 쿠버네티스 리소스들이 묶여있는 하나의 가상 공간(그룹)

**네임스페이스마다 권한 설정, 접근 제어 가능**
...
- 인증받지 않은 사용자를 포함함 모든 사용자가 읽기 권한으로 접근이 가능
- 관례적으로 만들어져 있지만 아무 리소스도 없고, 꼭 사용해야 하는 것도 아님
- **쿠버네티스 클러스터를 위해 예약된 공간**

**kube-system** 

- 쿠버네티스 클러스터의 리소스가 배치되는 네임스페이스
- 시스템이나 애드온(add on)이 사용하는 네임 스페이스

### 네임스페이스 사용 예
하나의 쿠버네티스 클러스터에 다양한 환경(개발, 테스트, 운영 환경 등)을 구현하고자 할때 하나의 네임스페이스 내에서는 오브젝트의 이름이 중복되는 것을 허용x

그러나 네임스페이스를 만들면 동일한 이름의 오브젝트 생성이 가능하다!(네임스페이스를 다르게 배치할 경우)

![Alt text](/statics/namespaceex.png)
...
<div class="form-layout">
        <blockquote>
        <p>Hello,  hostname-deploy-ns-746f55c57-pw2ns</p>       </blockquote>
</div>
...
default 네임스페이스에 생성된 테스트용 임시 파드는 production 네임스페이스의 서비스에 접근 불가


## 컨피그맵(configmap)
설정값을 파드에 전달하는 역할  
컨테이너에서는 컨피그맵 또는 시크릿을 파일 시스템으로 마운트해서 읽거나 환경 변수로 참조가능하다  

![Alt text](/statics/configmap.png)

같은 이미지로 빌드 했기 때문에 각 파드는 동일하고 둘다 설정파일, 인증서 키 값을 필요로 하지만 그 값은 서로 다르다  

네임스페이스로 구분해서 각 환경에서 필요로 하는 정보값을 넣어두는 것!  

일반적인 설정 정보를 담아 저장할 수 있는 쿠버네티스 오브젝트  

네임스페이스에 속하기 때문에 네임스페이스 별로 컨피그맵이 존재  

### 파드에서 컨피그맵을 사용하는 방법 1. 컨피그맵의 값을 파드의 환경 변수로 사용**

**컨피그맵에 정의된 모든 키-값 쌍을 파드의 환경 변수로 설정**  
**컨피그맵에 정의된 키 이름 대신 별칭으로 파드의 환경 변수를 설정**

### 파드에서 컨피그맵을 사용하는 방법 2. 컨피그맵의 값을 파드 내부의 파일로 마운트해서 사용

**컨피그맵의 모든 키-값 데이터를 파드에 마운트**  
**컨피그맵에 정의된 키 이름 대신 별칭으로 파드에 파일을 마운트**