...
- 컨테이너에서 VOLUME를 사용하면 컨테이너를 실행할 때 -v 또는 —volume옵션을 사용하여 호스트의 볼륨을 연결하거나 볼륨 컨테이너를 생성해 연결해 주어야 함
- VOLUME 명령을 사용하는 주요 목적
    - 데이터 보존(컨테이너가 종료되어도 유지될 수 있게)
    - 데이터 공유(동일한 데이터를 여러 컨테이너가 사용할 수 있게)
    - 데이터 컨테이너와 어플리케이션 코드를 분리(묶여 있으면 업그레이드 하는 것이 번거로울 수 있음)
